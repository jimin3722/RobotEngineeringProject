...
THETA_LIMIT = 150*math.pi/180


class inverse_kinematics:

    def __init__(self, l1, l2, l3, l4, a = 3.3):
    ...
    def calc_inv_to_ground_ver2(self,px,py,pz):
        ...
        try:
            ...
            u = self.l3*math.cos(theta_3 + theta_2) + self.l2*math.cos(theta_2)
            v = self.a
            pi = math.atan2(v/math.sqrt(u**2+v**2), u/math.sqrt(u**2+v**2))

            theta_1 = math.atan2(py/math.sqrt(u**2+v**2), px/math.sqrt(u**2+v**2)) - pi

            
            theta_4 = math.pi/2 - theta_2 - theta_3 #-20/180*math.pi
...
def main():
    ...
    qs = np.array(inv.calc_inv_to_ground(8,0,3))

    qs = np.array(inv.calc_inv_to_ground_ver2(px = 8, py = 0, pz = 3))
